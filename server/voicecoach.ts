...
const analyzeVoiceHandler: RequestHandler = async (req, res) => {
  try {
    ...
    const mergedFileUrl = `/api/voicecoach/merged-audio/${mergedFileName}`;
    ...
    res.json({
      ...reportData,
      reportUrl: `/voice-coach?report=${reportId}`
    });
...
router.use('/merged-audio', express.static(UPLOADS_DIR));

// Serve persistent report audio files
router.use('/report-audio', express.static(REPORTS_DIR));

// Setup routes
router.post('/analyze-voice', upload.array('audio'), analyzeVoiceHandler);
...
router.get('/report/:id', (req: Request, res: Response) => {
  ...
  reportData.mergedAudioUrl = `/api/voicecoach/report-audio/${reportId}.mp3`;
  res.json(reportData);
});
